...
# Gate-A SFT base evaluation launcher (no LoRA):
#   - LIBERO-Object tasks [5,6,7,8,9]
#   - LIBERO-Spatial tasks [0,1,2,3,4]
#   - LIBERO-10 ("long") tasks [3,4,5,6,7]
#
...
SPATIAL_MODEL_DIR="${SPATIAL_MODEL_DIR:-$REPO_ROOT/model/Openvla-oft-SFT-libero-spatial-traj1}"
LONG_MODEL_DIR="${LONG_MODEL_DIR:-$REPO_ROOT/model/Openvla-oft-SFT-libero10-trajall}"
...
echo "Run suites:       object=$RUN_OBJECT spatial=$RUN_SPATIAL long=$RUN_LONG"
echo "Model dirs:"
echo "  object:  $OBJECT_MODEL_DIR"
echo "  spatial: $SPATIAL_MODEL_DIR"
echo "  long:    $LONG_MODEL_DIR"
...
if [[ "$RUN_SPATIAL" == "1" ]]; then
  if check_model_dir "spatial" "$SPATIAL_MODEL_DIR"; then
    run_eval_suite \
      "libero_spatial" \
      "crl_experiment/libero_spatial_grpo_openvlaoft_eval_spatial" \
      "[0,1,2,3,4]" || OVERALL_EXIT_CODE=$?
  elif [[ $? -eq 1 ]]; then
    exit 1
  fi
fi
...
if [[ "$RUN_LONG" == "1" ]]; then
  if check_model_dir "long" "$LONG_MODEL_DIR"; then
    run_eval_suite \
      "libero_10_long" \
      "crl_experiment/libero_10_grpo_openvlaoft_eval_long" \
      "[3,4,5,6,7]" || OVERALL_EXIT_CODE=$?
  elif [[ $? -eq 1 ]]; then
    exit 1
  fi
fi
