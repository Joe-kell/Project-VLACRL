#!/bin/bash
### Usage: bash examples/mll_cluster/eval_embodiment_lora_scale.sh CHECKPOINT_LOCATION [CURRENT_LORA_SCALE] [PREVIOUS_LORA_COEFF] [STEP_NUMBER] [CONFIG_NAME]
### Example (single LoRA): bash examples/mll_cluster/eval_embodiment_lora_scale.sh logs/bcrl_logit/0.3/task_0 0.5
### Example (multi-LoRA): bash examples/mll_cluster/eval_embodiment_lora_scale.sh logs/naive_lora_multilora/task_2_coeff_0_9
### Example (multi-LoRA with current scale): bash examples/mll_cluster/eval_embodiment_lora_scale.sh logs/naive_lora_multilora/task_2_coeff_0_9 0.7
### Example (multi-LoRA with both overrides): bash examples/mll_cluster/eval_embodiment_lora_scale.sh logs/naive_lora_multilora/task_2_coeff_0_9 0.7 0.8
### Example (with custom step): bash examples/mll_cluster/eval_embodiment_lora_scale.sh logs/bcrl_logit/0.3/task_0 0.5 "" 20
### Example (libero_10): bash examples/mll_cluster/eval_embodiment_lora_scale.sh logs/bcrl_logit/0.3/task_0 0.5 "" "" mll_cluster/libero_10_grpo_openvlaoft_eval_long
### Note: CHECKPOINT_LOCATION should be relative to workspace root
###       For multi-LoRA checkpoints, PREVIOUS_LORA_COEFF is auto-extracted from path
###       CURRENT_LORA_SCALE: required for single LoRA, defaults to 1.0 for multi-LoRA

CHECKPOINT_LOCATION=$1
CURRENT_LORA_SCALE=$2
PREVIOUS_LORA_COEFF=$3
STEP_NUMBER=$4
CONFIG_NAME=${5:-mll_cluster/libero_spatial_grpo_openvlaoft_eval}

if [ -z "$CHECKPOINT_LOCATION" ]; then
    echo "ERROR: Missing required argument"
    echo "Usage: bash examples/mll_cluster/eval_embodiment_lora_scale.sh CHECKPOINT_LOCATION [CURRENT_LORA_SCALE] [PREVIOUS_LORA_COEFF] [STEP_NUMBER] [CONFIG_NAME]"
    exit 1
fi
...
# Check if this is a multi-LoRA checkpoint
IS_MULTILORA=false
EXTRACTED_COEFF=""
    ...
    if [ "$TASK_ID" -gt $FIRST_TASK_ID ]; then
        ...
        for prev_task in $(seq $FIRST_TASK_ID $((TASK_ID - 1))); do
            if [ "$prev_task" -eq $FIRST_TASK_ID ]; then
                prev_adapter_path="${WORKSPACE_ROOT}/logs/naive_lora/task_${prev_task}/checkpoints/global_step_${STEP_NUMBER}/actor"
            else
                prev_adapter_path="${WORKSPACE_ROOT}/logs/naive_lora_multilora/task_${prev_task}_coeff_${COEFFICIENT_PATH}/checkpoints/global_step_${STEP_NUMBER}/actor"
            fi
