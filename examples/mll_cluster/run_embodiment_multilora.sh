#!/bin/bash
### Usage: bash examples/mll_cluster/run_embodiment_multilora.sh TASK_ID_OR_RANGE [MERGE_COEFFICIENT] [CONFIG_NAME] [SEED]
### Example (single task): bash examples/mll_cluster/run_embodiment_multilora.sh 0
### Example (single task): bash examples/mll_cluster/run_embodiment_multilora.sh 1 0.8
### Example (with seed):   bash examples/mll_cluster/run_embodiment_multilora.sh 0 "" "" 42
### Example (range):       bash examples/mll_cluster/run_embodiment_multilora.sh "0,3" 0.8
###
### Behavior:
###   - First task ID: Train first LoRA adapter on top of base model (same as naive LoRA).
###   - Subsequent tasks: Load and merge ALL previous task adapters, then create a NEW LoRA adapter.
### MERGE_COEFFICIENT (optional, default=1.0): Coefficient applied when merging previous LoRA adapters (0.0 to 1.0)
...
for TASK_ID in $(seq $TASK_START $TASK_END); do
    ...
    if [ "$TASK_ID" -gt $FIRST_TASK_ID ]; then
        ...
        for prev_task in $(seq $FIRST_TASK_ID $((TASK_ID - 1))); do
            ...
            [ -z "$CONFIG_TAG" ] && prev_log_dir_transformed="$prev_log_dir" || prev_log_dir_transformed=$(inject_config_tag_into_log_path "$prev_log_dir" "$CONFIG_TAG")
...
echo "========================================="
if [ "$IS_RANGE" = true ]; then
    [ $OVERALL_EXIT_CODE -eq 0 ] && echo "All Multi-LoRA tasks (${TASK_START} to ${TASK_END}) completed successfully!" || echo "Multi-LoRA range training failed."
else
    [ $OVERALL_EXIT_CODE -eq 0 ] && echo "Task $TASK_START completed successfully (Multi-LoRA)" || echo "Task $TASK_START failed (Multi-LoRA)"
fi
