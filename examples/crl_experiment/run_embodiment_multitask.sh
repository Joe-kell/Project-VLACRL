...
if [ -z "$TASK_IDS_STR" ]; then
    ...
    echo "Example (with checkpoint): bash examples/crl_experiment/run_embodiment_multitask.sh \"0,2,4\" ./logs/multitask/tasks_0_2_4_seed1234/checkpoints/global_step_10/actor"
    echo "Example (with seed): bash examples/crl_experiment/run_embodiment_multitask.sh \"0,2,4\" \"\" \"\" \"\" 42"
    exit 1
fi
...
if [ -z "$LOG_DIR" ]; then
    if [ -n "$CHECKPOINT_PATH" ]; then
        ...
        if [[ "$CHECKPOINT_PATH" =~ global_step_([0-9]+) ]]; then
            ...
            LOG_DIR="./logs/multitask/tasks_${TASK_DIR_STR}_from_checkpoint_step_${SOURCE_STEP}_seed${SEED}"
        else
            # Fallback: use a generic name
            LOG_DIR="./logs/multitask/tasks_${TASK_DIR_STR}_from_checkpoint_seed${SEED}"
        fi
    else
        # Standard case: use task IDs
        LOG_DIR="./logs/multitask/tasks_${TASK_DIR_STR}_seed${SEED}"
    fi
